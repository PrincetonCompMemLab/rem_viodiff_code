...
# data path to SVD dataset 
svd_dir = '/path/to/study/directory/'
svd_bids_dir = '/path/to/bids/directory/'

# constants for the svd dataset (localizer)
svd_label_dict = {1: "Faces", 2: "Scenes", 3: "Objects", 0: "Rest"}
svd_all_ROIs = []
svd_TR = 1.5
svd_hrf_lag = 4.5  # In seconds what is the lag between a stimulus onset and the peak bold response

run_names = ['localizer', 'study', 'postScenes', 'familiarization', 'reward', 'decision', 'postFaces']
n_runs = [2, 6, 1, 2, 4, 2, 1]
...
def load_svd_stim_labels(sub):
    """load the stimulus labels for the svd data
    Parameters 
    ----------
    ...
    for run in range(1, svd_n_runs + 1):
        in_file = (svd_data_dir + '/behavioral/regressor/' + '%s_ses-00_task-localizer_regressor-noshift-trimmed_run-0%d.mat' % (sub, run))
...
def load_svd_mask(ROI_name, sub):
    ...
    the requested mask
    """    
    #assert ROI_name in svd_all_ROIs
    maskdir = (svd_bids_dir + "derivatives/firstlevel/" + sub + "/masks/")
    # load the mask
...
#         for run in range(1, svd_n_runs + 1):
#             # load fMRI data 
#             epi_data = load_svd_epi_data(subject_name, run)
#             # mask the data 
...
def load_data(directory, subject_name, mask_name='', num_runs=2, zscore_data=False):
    ...
    if mask_name is '':
        ...
        mask = load_svd_mask(mask_name, subject_name)
