function VioDiffCon_practice_decision(SN,NM,block,fMRI,version)
global deburg
...
%% load images
[textures, imgsize] = VioDiffCon_load_images_practice_decisionphase(mainWindow, data, data_rows, fMRI); %LM added fMRI has an input so that variable carries over to VioDiffCon_VarConf
%% Setups
...
instructString{5} = 'Press 1 when you are ready to start!';
...
save([output_dir '/practice_decision_' SN '_block' num2str(block) '_' NM], 'data', 'decisionphase_winnings', 'data_rows', 'version');
