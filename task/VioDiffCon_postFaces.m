function VioDiffCon_postFaces(SN,NM,block,fMRI,version)
global deburg
if nargin == 0
    SN = '999'; NM = 'SVD'; block = 1; fMRI = 0; deburg = 0; version='997';
end%if isempty
...
%load([out_mat_dir '/postFaces_mat_' SN], 'postStudy_faces_mat');
load([out_mat_dir '/postFaces_mat_' version], 'postStudy_faces_mat');
load([out_mat_dir '/postStudy_mat_row'], 'postStudy_mat_row');
data = postStudy_faces_mat{block};
...
% show instructions
instructString{1} = 'This run is FACES ONLY';
instructString{2} = ' ';
instructString{3} = 'Fixate on the circle in the center of the screen';
instructString{4} = sprintf('For each face image, report whether it is %s(left) or %s(right)', left_face, right_face);
instructString{5} = 'Press the left button when you are ready to start!';

% if mod(str2double(SN),2) == 1
%     instructString{2} = 'For each face image, report whether it is female(left) or male(right)';
% else
%     instructString{2} = 'For each face image, report whether it is male(left) or female(right)';
% end%if mod
...
for trial=1:nTrials_thisrun
    ...
    xOrder = data(data_rows.order,trial);%A or B
...
%     Screen('Flip',mainWindow);
    ...
    save([output_dir '/data_postFaces_' SN '_block' num2str(block) '_' NM], 'data', 'data_rows', 'keypress', 'version');
end % trial loop

% save([output_dir '/data_postFaces_' SN '_block' num2str(block) '_' NM], 'data', 'data_rows', 'keypress');
...
instructString{1} = 'Congratulations! You completed run 10 of 10.';
instructString{2} = ' ';
...
if deburg
    ...
    save([server_dir '/data_postFaces_' SN '_block' num2str(block) '_' NM], 'data', 'data_rows', 'keypress', 'version');
end
%%
