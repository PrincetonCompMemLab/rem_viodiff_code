function VioDiffCon_reward(SN,NM,block,fMRI,version)
global deburg
if nargin == 0
    SN = '999'; NM = 'SVD'; block = 3; fMRI = 0; deburg = 0; version='999';
end%if isempty
...
% % DON'T FORGET TO UPDATE LINE 196 AS WELL!!!
...
data = reward_mat{cycle}{run};
data_rows = reward_mat_row;
rewardphase_winnings{cycle}{run} = [];
track_winnings = 0;
...
instructString{4} = sprintf('For each scene image, predict if it leads to a %s(left) or %s(right) outcome', left_reward, right_reward);
...
%%
runLength = max(data(data_rows.time,:)) + trialDur_reward + post_disdaq;
...
for trial=1:nTrials_thisrun
    ...
    %imageRect_outcome = [0,0,imageSize*1.64,imageSize*0.82]; %make rectangle instead of square
...
%     check_screenclear = screen_cleared - winning_onset
    ...
    save([output_dir '/data_reward_' SN '_block' num2str(block) '_' NM], 'data', 'rewardphase_winnings', 'data_rows', 'keypress', 'version');
end % trial loop
